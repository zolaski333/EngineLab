...
bool runCatalogAcceptance(const std::filesystem::path& root) {
    ...
    for (const auto& entry : catalog.entries) {
        ...
        for (int step = 0; step < static_cast<int>(5.0 / dt); ++step) {
            const auto time = static_cast<double>(step) * dt;
            const auto frame = simulator.step(dt, { true, time < 1.5, 1.0, 0.0 });
            const auto& state = frame.state;
            ...
            if (state.rpm > std::max(1'200.0, config.idleRpm * 1.25)
                    && state.rpm < config.ignition.revLimitRpm * 0.82) {
        ...
        std::cout << "Catalog gate: " << config.name << " peak=" << peakRpm
                  << " rpm, AFR error=" << meanAfrError << ", FMEP=" << meanFmep
                  << " bar, boost=" << peakBoost << '\n';
        if (engineFailed) {
...
int main(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        else { std::cerr << "usage: EngineLabComparisonHarness [--output dir] [--reference summary.csv] [--catalog-root dir]\n"; return 2; }
    }
    ...
    for (const auto& scenario : scenarios) {
        ...
        results.emplace(scenario.name, runScenario(scenario, output));
    }
    ...
    return runCatalogAcceptance(catalogRoot) ? 0 : 1;
}
